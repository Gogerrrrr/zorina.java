...
public class RestaurantView extends JFrame {
    ...
    private void showOrderDialog() {
        ...
        switch (choice) {
            case 0:
                showPlaceOrderDialog(); // Должен вызывать исправленный метод
                break;
            case 1:
    ...
    private void showPlaceOrderDialog() {
        String[] options = model.getMenu().stream()
                .map(dish -> "🍽️ " + dish.getName() + " (-" + dish.getPrice() + "$, +5 очков)")
                .toArray(String[]::new);

        String choice = (String) JOptionPane.showInputDialog(
                this,
                "Какое блюдо заказать на кухне?\n\n" +
                        "💡 Заказ будет стоить денег, но принесёт очки\n" +
                        "   Блюдо приготовится и будет ждать на кухне",
                "📝 Заказ на кухне",
    ...
    private class GamePanel extends JPanel {
        ...
        protected void paintComponent(Graphics g) {
            ...
            drawRestaurant(g2d);
            drawKitchen(g2d);
            drawTrashBin(g2d); // ДОБАВИТЬ ЭТУ СТРОКУ
        ...
        private void drawTrashBin(Graphics2D g2d) {
            int x = model.getTrashBinX();
            int y = model.getTrashBinY();

            // Основание бака
            g2d.setColor(new Color(80, 80, 80));
            g2d.fillRect(x - 20, y - 30, 40, 50);

            // Крышка
            g2d.setColor(new Color(60, 60, 60));
            g2d.fillRect(x - 25, y - 35, 50, 10);

            // Ободок
            g2d.setColor(new Color(100, 100, 100));
            g2d.drawRect(x - 20, y - 30, 40, 50);
            g2d.drawRect(x - 25, y - 35, 50, 10);

            // Значок мусора
            g2d.setColor(Color.WHITE);
            g2d.setFont(new Font("Arial", Font.BOLD, 20));
            g2d.drawString("🗑️", x - 12, y + 5);

            // Подпись
            g2d.setColor(Color.BLACK);
            g2d.setFont(new Font("Arial", Font.PLAIN, 10));
            g2d.drawString("Мусорка", x - 18, y + 25);
        }
